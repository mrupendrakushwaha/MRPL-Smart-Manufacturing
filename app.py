...
def init_db():
    ...
    def product_image(*parts):
        """Return a local image path from the deployed repository."""
        path = APP_DIR.joinpath(*parts)
        return str(path) if path.exists() else ""

    def product_images(folder, filenames):
        """Return all existing local product images as a comma-separated value."""
        paths = []
...
PRODUCT_IMAGE_ROOT = Path(__file__).resolve().parent
        ...
        for i, (_, row) in enumerate(cat_products.iterrows()):
            with cols[i % 3]:
                with st.container(border=True):
                    # Use the database image first. If the deployed database is
                    # old/blank, fall back to the actual repository files.
                    image_urls = [u.strip() for u in str(row["image_url"] or "").split(",") if u.strip()]
                    valid_images = [u for u in image_urls if Path(u).is_file()]

                    if not valid_images:
                        fallback_paths = [
                            str(PRODUCT_IMAGE_ROOT / rel_path)
                            for rel_path in PRODUCT_IMAGE_MAP.get(row["product_name"], [])
                        ]
                        valid_images = [u for u in fallback_paths if Path(u).is_file()]
