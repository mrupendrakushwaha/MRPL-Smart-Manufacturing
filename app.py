...
st.markdown(
    .stApp::before {
        ...
        inset: -18px;
        z-index: -1;
        background: url("data:image/jpeg;base64,{Mrpl}") center center / cover no-repeat;
        filter: blur(9px);
    ...
    div[data-testid="stForm"] {
        ...
        border: 1px solid rgba(72, 147, 205, 0.35);
        border-radius: 14px;
    }
    div[data-testid="stVerticalBlockBorderWrapper"] {
        background: rgba(7, 27, 48, 0.78);
        border-color: rgba(72, 147, 205, 0.35) !important;
        border-radius: 14px;
    }
    </style>
    """,
...
# Images are stored in this project's GitHub repository.
# ============================================================

GITHUB_RAW_BASE = "https://raw.githubusercontent.com/mrupendrakushwaha/MRPL-Smart-Manufacturing/main/"

PRODUCT_IMAGE_PATHS = {
    # Shaped Products
    "Fireclay & High Alumina Bricks": "Shaped-Product/Fireclay and High Alumina Bricks.jpg",
    "Pre-Cast Pre-Fired (PCPF) Blocks": "Shaped-Product/Pre-Cast Pre-Fired (PCPF) Blocks.jpg",
    ...
    "Fireclay & High Alumina Mortars": [
        "Unshaped Product/Fire Clay & High Alumina Mortars/Maxset50finemonolithic.jpg",
        "Unshaped Product/Fire Clay & High Alumina Mortars/Maxset50monolithic.jpg",
        "Unshaped Product/Fire Clay & High Alumina Mortars/Mortar75.jpg",
    ],
}
...
def _raw_github_url(path):
    ...
    return GITHUB_RAW_BASE + "/".join(quote(part, safe="") for part in path.split("/"))


def _product_image_value(product_name):
    paths = PRODUCT_IMAGE_PATHS.get(product_name)
    if not paths:
        return ""
    if isinstance(paths, str):
        return _raw_github_url(paths)
    # Store the first image as the card image. The product detail can still
    # be expanded later without changing the database schema.
    return _raw_github_url(paths[0])


def sync_product_images():
    """Keep image_url in sync with the GitHub product-image folders."""
    conn = get_connection()
    cur = conn.cursor()
    for product_name in PRODUCT_IMAGE_PATHS:
        image_url = _product_image_value(product_name)
        cur.execute(
        ...
        for i, (_, row) in enumerate(cat_products.iterrows()):
            with cols[i % 3]:
                with st.container(border=True):
                    ...
                    image_paths = PRODUCT_IMAGE_PATHS.get(product_name, [])
                    if isinstance(image_paths, str):
                        image_paths = [image_paths]

                    if image_paths:
                        # Main product image
                        st.image(_raw_github_url(image_paths[0]), use_container_width=True)

                        # Show additional product images where the GitHub folder contains them.
                        if len(image_paths) > 1:
                            with st.expander("🖼️ More product images"):
                                gallery_cols = st.columns(min(3, len(image_paths)))
                                for j, image_path in enumerate(image_paths):
                                    with gallery_cols[j % len(gallery_cols)]:
                                        st.image(_raw_github_url(image_path), use_container_width=True)
