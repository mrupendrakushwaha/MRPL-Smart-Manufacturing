...
DB_PATH = "mrpl.db"
CLEAN_START_VERSION = "professional_single_admin_v1"
...
def init_db():
    ...
    if cur.fetchone() is None:
        ...
        if keep_admin_id is not None:
            ...
            cur.execute(
                "UPDATE users SET role='Admin', employee_id=NULL, is_active=1 WHERE id=?",
                (keep_admin_id,),
            )
        else:
...
if not st.session_state.logged_in:
    ...
    with left:
        st.markdown(
            ...
            <div style='padding:40px 10px 10px 10px;'>
                ...
                <div class='login-title'>MRPL Smart Manufacturing</div>
                <div class='login-subtitle'>Secure administrator portal for manufacturing operations.</div>
                <div class='info-card'>
                    <b>Administrator access only</b><br>
                    <span style='color:#6b7280;font-size:13px;'>This system is configured with one administrator account. No employee, manager or member accounts are created automatically.</span>
                </div>
            </div>
            """,
            unsafe_allow_html=True,
    ...
    with right:
        ...
        st.caption("Sign in to manage MRPL operations.")
        with st.form("login_form"):
            username = st.text_input("Username", placeholder="Administrator username")
        ...
        st.divider()
        st.markdown("### Public Product Catalogue")
        st.caption("Product catalogue is available without administrator login.")
...
with st.sidebar:
    ...
    selected_module = st.radio("NAVIGATION", modules, label_visibility="visible")
