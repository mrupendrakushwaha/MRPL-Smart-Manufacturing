...
# ============================================================
# MRPL PRO UI THEME — dark industrial + clean e-commerce cards
# ============================================================
...
try:
    ...
    MRPL_BG_DATA = ""
MRPL_BG_CSS = f'url("data:image/jpeg;base64,{MRPL_BG_DATA}")' if MRPL_BG_DATA else 'none'
...
:root{
    --mrpl-accent:#2fd6c0;
    --mrpl-accent-2:#5aa8ff;
    --mrpl-bg-1:#080f1c;
    --mrpl-bg-2:#0d1830;
    --mrpl-surface:rgba(17,28,48,.72);
    --mrpl-surface-solid:#101b30;
    --mrpl-border:rgba(120,150,190,.16);
    --mrpl-text:#eef3f9;
    --mrpl-text-dim:#93a3ba;
}

html, body, [class*="css"] {font-family:'Plus Jakarta Sans', sans-serif;}

.stApp {
    background: linear-gradient(160deg, rgba(6,12,24,.93) 0%, rgba(9,16,32,.90) 45%, rgba(6,12,24,.95) 100%), __MRPL_BG__;
    background-size:cover; background-position:center; background-attachment:fixed;
    color:var(--mrpl-text);
}
.block-container {max-width:1480px;padding:1.4rem 2.1rem 3rem;}
header[data-testid="stHeader"] {background:rgba(255,255,255,.97); backdrop-filter:blur(6px);}
h1,h2,h3,h4 {font-family:'Sora', sans-serif; letter-spacing:-.01em;}
::-webkit-scrollbar{width:9px;height:9px;}
::-webkit-scrollbar-thumb{background:#24354f;border-radius:8px;}
::-webkit-scrollbar-track{background:transparent;}

/* SIDEBAR */
section[data-testid="stSidebar"] {background:linear-gradient(195deg,#0a1223 0%,#070d1a 100%) !important;border-right:1px solid var(--mrpl-border);}
section[data-testid="stSidebar"] > div {background:transparent !important;}
section[data-testid="stSidebar"] .block-container {padding:1.1rem .95rem 2rem;}

.mrpl-side-brand {display:flex;align-items:center;gap:12px;padding:6px 8px 20px;border-bottom:1px solid var(--mrpl-border);margin-bottom:18px;}
.mrpl-logo {width:46px;height:46px;border-radius:14px;display:flex;align-items:center;justify-content:center;background:linear-gradient(135deg,var(--mrpl-accent),var(--mrpl-accent-2));font-size:22px;box-shadow:0 8px 20px rgba(47,214,192,.25);}
.mrpl-side-title {font-family:'Sora',sans-serif;font-size:18px;font-weight:800;color:#fff;line-height:1.1;}
.mrpl-side-sub {font-size:11px;color:var(--mrpl-text-dim);margin-top:4px;}

.mrpl-user-card {background:rgba(47,214,192,.07);border:1px solid rgba(47,214,192,.28);border-radius:16px;padding:14px 16px;margin-bottom:12px;}
.mrpl-user-name {font-size:16px;font-weight:800;color:#fff;}
.mrpl-user-role {font-size:11.5px;font-weight:700;color:var(--mrpl-accent);margin-top:4px;text-transform:uppercase;letter-spacing:.4px;}

.mrpl-nav-title {font-size:11px;font-weight:800;color:#5c7188;text-transform:uppercase;letter-spacing:1.3px;margin:20px 4px 10px;}
section[data-testid="stSidebar"] [data-testid="stRadio"] > label {color:#c8d6e6 !important;font-weight:700 !important;font-size:13px !important;}
section[data-testid="stSidebar"] [role="radiogroup"] {gap:5px;}
section[data-testid="stSidebar"] [role="radiogroup"] > label {background:rgba(255,255,255,.03);border:1px solid transparent;border-radius:12px;padding:9px 12px;margin:0 !important;transition:all .15s ease;}
section[data-testid="stSidebar"] [role="radiogroup"] > label:hover {background:rgba(47,214,192,.09);border-color:rgba(47,214,192,.35);}
section[data-testid="stSidebar"] .stButton > button {background:transparent;border:1px solid rgba(255,90,90,.4);color:#ff9a9a;border-radius:12px;font-weight:700;box-shadow:none;}
section[data-testid="stSidebar"] .stButton > button:hover {background:rgba(255,90,90,.1);border-color:#ff5a5a;color:#fff;}

/* MAIN APP HEADER */
.mrpl-appbar {display:flex;align-items:center;justify-content:space-between;gap:15px;background:var(--mrpl-surface);backdrop-filter:blur(14px);border:1px solid var(--mrpl-border);border-radius:20px;padding:20px 26px;margin-bottom:20px;box-shadow:0 14px 34px rgba(0,0,0,.28);}
.mrpl-appbar-title {font-family:'Sora',sans-serif;font-size:25px;font-weight:800;color:#fff;}
.mrpl-appbar-sub {font-size:12px;color:var(--mrpl-text-dim);margin-top:5px;}
.mrpl-status {display:flex;align-items:center;gap:8px;background:rgba(47,214,192,.08);border:1px solid rgba(47,214,192,.3);padding:8px 14px;border-radius:999px;color:var(--mrpl-accent);font-size:12px;font-weight:700;}
.mrpl-dot {width:7px;height:7px;border-radius:50%;background:var(--mrpl-accent);display:inline-block;box-shadow:0 0 8px var(--mrpl-accent);}

/* CARDS / METRICS */
div[data-testid="stVerticalBlockBorderWrapper"] {background:var(--mrpl-surface-solid);backdrop-filter:blur(12px);border:1px solid var(--mrpl-border);border-radius:18px;box-shadow:0 12px 30px rgba(0,0,0,.35);}
div[data-testid="stMetric"] {background:linear-gradient(150deg, rgba(19,33,58,.97), rgba(13,23,42,.97));border:1px solid var(--mrpl-border);border-radius:16px;padding:16px 18px;box-shadow:0 8px 20px rgba(0,0,0,.35);}
div[data-testid="stMetricLabel"] {color:#cfe0f0 !important;font-weight:700 !important;}
div[data-testid="stMetricLabel"] p {color:#cfe0f0 !important;font-weight:700 !important;}
div[data-testid="stMetricValue"] {color:#fff !important;font-weight:800 !important;font-family:'Sora',sans-serif;}

/* TABS */
button[data-baseweb="tab"] {font-weight:700 !important;color:var(--mrpl-text-dim) !important;}
button[data-baseweb="tab"][aria-selected="true"] {color:var(--mrpl-accent) !important;}
div[data-baseweb="tab-highlight"] {background-color:var(--mrpl-accent) !important;}

/* WIDGET LABELS (Employee Id, Employee Name, Status, etc.) */
...
.stMarkdown, .stMarkdown p, .stMarkdown li, .stCaption, [data-testid="stCaptionContainer"] {color:var(--mrpl-text) !important;}

/* INPUTS / BUTTONS */
div[data-baseweb="input"],div[data-baseweb="select"] > div,textarea,input,
.stDateInput input, .stNumberInput input, .stTextInput input, .stTextArea textarea {
...
div[data-baseweb="select"] *, div[data-baseweb="input"] * {color:#f2f7fb !important;}
input::placeholder, textarea::placeholder {color:#8fa2b8 !important;opacity:1;}
/* number input +/- steppers */
div[data-testid="stNumberInputStepUp"], div[data-testid="stNumberInputStepDown"] {background:rgba(255,255,255,.06) !important;color:#f2f7fb !important;}

/* SELECT DROPDOWN POPOVER — BaseWeb renders this in a portal outside .stApp,
   so it needs its own (unscoped) rule or it falls back to the light theme */
ul[data-baseweb="menu"], div[data-baseweb="popover"] ul[role="listbox"] {
    background:var(--mrpl-surface-solid) !important;
    border:1px solid var(--mrpl-border) !important;
...
ul[data-baseweb="menu"] li:hover, li[role="option"]:hover, li[aria-selected="true"] {
    background:rgba(47,214,192,.16) !important;
    color:#fff !important;
}
.stButton > button,.stLinkButton > a,.stDownloadButton > button {min-height:42px;border-radius:12px;font-weight:700;color:#04202a !important;border:none;background:linear-gradient(120deg,var(--mrpl-accent),var(--mrpl-accent-2));box-shadow:0 8px 18px rgba(47,214,192,.22);transition:transform .12s ease, box-shadow .12s ease;}
.stButton > button:hover,.stLinkButton > a:hover {transform:translateY(-1px);box-shadow:0 10px 22px rgba(47,214,192,.32);}
.stButton > button[kind="secondary"] {background:rgba(255,255,255,.05);color:var(--mrpl-text) !important;border:1px solid var(--mrpl-border);box-shadow:none;}

/* DATAFRAMES */
div[data-testid="stDataFrame"] {border-radius:14px;overflow:hidden;border:1px solid var(--mrpl-border);}

/* E-COMMERCE */
.store-shell {margin-top:4px;}
.store-topbar {display:flex;align-items:center;justify-content:space-between;gap:16px;background:rgba(255,255,255,.97);border-radius:18px;padding:14px 20px;margin:0 0 16px;box-shadow:0 10px 26px rgba(0,0,0,.20);border:1px solid #e7ecf1;}
.store-brand {font-family:'Sora',sans-serif;font-size:21px;font-weight:800;color:#0d2136;}
.store-tagline {font-size:11px;color:#7c8a9a;margin-top:3px;}
.store-badge-right {font-size:11px;font-weight:800;color:#0c9a86;background:#e7faf6;border:1px solid #c6f0e6;border-radius:999px;padding:7px 12px;}
.store-hero {min-height:300px;display:flex;align-items:center;padding:44px;border-radius:22px;overflow:hidden;background-image:linear-gradient(100deg,rgba(6,14,26,.92),rgba(6,14,26,.45)),__MRPL_BG__;background-size:cover;background-position:center;box-shadow:0 16px 36px rgba(0,0,0,.28);margin-bottom:22px;}
.store-hero-content {max-width:740px;}
.store-hero-title {color:#fff;font-family:'Sora',sans-serif;font-size:40px;line-height:1.12;font-weight:800;margin:10px 0 12px;}
.store-hero-text {color:#d3e0ea;font-size:15px;line-height:1.65;max-width:640px;}
.store-pill {display:inline-block;background:rgba(47,214,192,.18);border:1px solid rgba(47,214,192,.4);color:var(--mrpl-accent);padding:7px 13px;border-radius:999px;font-size:11px;font-weight:800;letter-spacing:.4px;}
.store-section-title {font-family:'Sora',sans-serif;font-size:26px;font-weight:800;color:#fff;margin:10px 0 3px;}
.store-section-subtitle {font-size:13px;color:var(--mrpl-text-dim);margin-bottom:16px;}
.product-category {font-size:10px;font-weight:800;letter-spacing:.8px;text-transform:uppercase;color:var(--mrpl-accent);margin-top:8px;}
.product-name {font-size:17px;font-weight:800;color:#fff;margin:5px 0;}
.product-description {font-size:12.5px;line-height:1.6;color:var(--mrpl-text-dim);}

@media(max-width:768px){
    .block-container{padding:.9rem .7rem 2rem;}
    .mrpl-appbar{padding:16px;flex-wrap:wrap;}
    .mrpl-appbar-title{font-size:19px;}
    .mrpl-status{display:none;}
    .store-hero{min-height:270px;padding:26px;}
    .store-hero-title{font-size:28px;}
    .store-hero-text{font-size:13px;}
    .store-topbar{padding:12px 14px;}
    .store-badge-right{display:none;}
}
</style>
"""
st.markdown(MRPL_CSS.replace("__MRPL_BG__", MRPL_BG_CSS), unsafe_allow_html=True)
...
def render_products_page():
    ...
    st.markdown("""<div class="store-topbar"><div><div class="store-brand">🏭 MRPL Products</div><div class="store-tagline">Mahakoshal Refractories • Industrial Refractory Solutions</div></div><div class="store-badge-right">PRODUCT CATALOG</div></div><div class="store-hero"><div class="store-hero-content"><span class="store-pill">🔥 INDUSTRIAL REFRACTORY PRODUCTS</span><div class="store-hero-title">Mahakoshal Refractories</div><div class="store-hero-text">Explore shaped and unshaped refractory products for demanding industrial applications.</div></div></div>""", unsafe_allow_html=True)
...
if not st.session_state.logged_in:
    st.markdown("""<div class="store-shell"><div class="store-topbar"><div><div class="store-brand">🏭 MRPL</div><div class="store-tagline">Mahakoshal Refractories • Smart Manufacturing Platform</div></div><div class="store-badge-right">● BUSINESS SYSTEM</div></div><div class="store-hero"><div class="store-hero-content"><span class="store-pill">INDUSTRIAL REFRACTORY & MANUFACTURING</span><div class="store-hero-title">MRPL Smart Manufacturing System</div><div class="store-hero-text">A professional digital workspace for manufacturing operations, quality control, inventory, orders and the MRPL product catalog.</div></div></div></div>""", unsafe_allow_html=True)
...
st.sidebar.markdown("""
<div class="mrpl-side-brand"><div class="mrpl-logo">🏭</div><div><div class="mrpl-side-title">MRPL</div><div class="mrpl-side-sub">SMART MANUFACTURING</div></div></div>
""", unsafe_allow_html=True)
st.sidebar.markdown(f"""<div class="mrpl-user-card"><div class="mrpl-user-name">👤 {username}</div><div class="mrpl-user-role">{user_role} • Secure Staff Area</div></div>""", unsafe_allow_html=True)
if st.sidebar.button("🚪  Logout", width="stretch"):
    st.session_state.logged_in = False
    st.session_state.user = None
    st.rerun()
st.sidebar.markdown('<div class="mrpl-nav-title">📋 Navigation</div>', unsafe_allow_html=True)
...
selected_module = st.sidebar.radio("Select Module", allowed_modules, label_visibility="collapsed") if allowed_modules else None

if not allowed_modules:
    st.sidebar.warning("No modules assigned to this role.")

st.markdown("""<div class="mrpl-appbar"><div><div class="mrpl-appbar-title">🏭 MRPL Smart Manufacturing System</div><div class="mrpl-appbar-sub">Manufacturing • Quality • Inventory • Orders • Workforce</div></div><div class="mrpl-status"><span class="mrpl-dot"></span> System Online</div></div>""", unsafe_allow_html=True)
