...
""", unsafe_allow_html=True)
    ...
    # --------------------------------------------------------
    tab_admin, tab_other, tab_edit = st.tabs(
        ["➕ Add Admin", "➕ Add Manager / Employee", "✏️ Edit Account"]
    )
    ...
    with tab_edit:
        st.subheader("✏️ Edit Existing Account")
        ...
        if visible_users.empty:
            ...
            edit_user_id = int(
                re.search(r"ID: (\d+)", edit_selected_label).group(1)
            )
            edit_user_row = visible_users[visible_users["id"] == edit_user_id].iloc[0]

            with st.form("edit_account_form"):
                edited_username = st.text_input(
                    "Username",
                    value=str(edit_user_row["username"])
                )

                role_options = ["Admin", "Manager", "Employee"]
                current_role_index = (
                    role_options.index(edit_user_row["role"])
                    if edit_user_row["role"] in role_options
                    else 0
                )
                ...
                st.caption("Leave password fields blank to keep the current password unchanged.")
                edited_password = st.text_input("New Password (optional)", type="password")
                edited_password2 = st.text_input("Confirm New Password", type="password")

                if st.form_submit_button("Save Changes", type="primary", width="stretch"):
                    if not edited_username.strip():
                        st.error("Username cannot be empty.")
                    elif edited_password and edited_password != edited_password2:
                        st.error("Passwords do not match.")
                        ...
                        try:
                            if edited_password:
                                run_query(
                                    ...
                                    (edited_username.strip(), edited_role, hash_password(edited_password), edit_user_id)
                                )
                            else:
                                run_query(
                                    "UPDATE users SET username = ?, role = ? WHERE id = ?",
                                    (edited_username.strip(), edited_role, edit_user_id)
                                )
