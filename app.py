...
# ============================================================
# INDUSTRIAL DARK UI - visual layer only
# ============================================================
st.markdown(
    """
    <style>
    ...
    h1, h2, h3, h4, p, label, .stMarkdown, [data-testid="stCaptionContainer"] {
    ...
    div[data-testid="stVerticalBlockBorderWrapper"] {
        background: var(--mrpl-card);
        border: 1px solid var(--mrpl-border);
        border-radius: 16px;
        box-shadow: 0 6px 24px rgba(16, 38, 58, .06);
    }
    ...
    div[data-testid="stMetric"] {
        ...
        border-radius: 14px;
        padding: 14px 16px;
        box-shadow: 0 5px 18px rgba(16, 38, 58, .05);
    }
    div[data-testid="stMetricLabel"] { color: var(--mrpl-muted) !important; }
    div[data-testid="stMetricValue"] { color: var(--mrpl-navy) !important; }
    ...
    .hero {
        background: linear-gradient(135deg, #0b1f33 0%, #174b72 65%, #1266a3 100%);
        border-radius: 22px;
        padding: 38px 42px;
        color: white;
        margin: 8px 0 24px;
    ...
    .hero p { opacity: .88; max-width: 800px; }
    ...
    .stDownloadButton > button {
        ...
        background: white;
        color: var(--mrpl-navy) !important;
        box-shadow: none;
    }
    .stButton > button:hover, .stLinkButton > a:hover {
        border-color: var(--mrpl-blue);
        color: var(--mrpl-blue) !important;
    }
    ...
    hr { border-color: var(--mrpl-border) !important; }
...
def render_products_page():
    st.markdown(
        """
        <div class="hero">
            <h1>Mahakoshal Refractories</h1>
            <p>
                Explore our refractory product portfolio for high-temperature
                industrial applications. Browse by category and contact our team
                for product enquiries and technical requirements.
            </p>
        </div>
        """,
        unsafe_allow_html=True
    ...
    search_col, category_col = st.columns([2.2, 1], gap="medium")
    with search_col:
    ...
    with category_col:
        categories = ["All Products"] + sorted(
    ...
    if search.strip():
        mask = (
            filtered["product_name"].astype(str).str.contains(
                search.strip(), case=False, na=False
            )
            | filtered["description"].astype(str).str.contains(
                search.strip(), case=False, na=False
            )
        )
    ...
    st.markdown(
        f'<div class="section-title">Product Catalogue <span style="font-size:.85rem;color:#687787;font-weight:500;">'
        f'{len(filtered)} products</span></div>',
        unsafe_allow_html=True
    )

    if filtered.empty:
        st.info("No matching products found. Try another search or category.")
        return

    for cat in filtered["category"].dropna().unique():
        cat_products = filtered[filtered["category"] == cat]
        ...
        for i, (_, row) in enumerate(cat_products.iterrows()):
            with cols[i % 3]:
                with st.container(border=True):
                    image_urls = [
                        u.strip()
                        for u in str(row["image_url"] or "").split(",")
                        if u.strip()
                    ]
                    valid_images = [
                        u for u in image_urls if Path(u).is_file()
                    ]
                    ...
                    if valid_images:
                        ...
                        st.markdown(
                            ...
                            <div style="
                                ...
                                background:#f8fafc;border:1px solid #edf1f5;
                                display:flex;align-items:center;justify-content:center;
                                color:#8a98a8;font-weight:600;">
                                Product Image
                            </div>
                            """,
                            unsafe_allow_html=True
                    ...
                    st.markdown(
                        f'<span class="category-pill">{cat}</span>',
                        unsafe_allow_html=True
                    )
                    st.markdown(
                        f'<div class="product-card-title">{row["product_name"]}</div>',
                        unsafe_allow_html=True
                    )
                    st.markdown(
                        f'<div class="product-card-desc">{row["description"] or ""}</div>',
                        unsafe_allow_html=True
                    )

                    if st.button(
                        "View Product",
                        key=f"view_product_{row['id']}",
                        use_container_width=True
                    ):
                        st.session_state[f"selected_product_{row['id']}"] = True

                    if st.session_state.get(f"selected_product_{row['id']}", False):
                        st.info(
                            f"**{row['product_name']}**\n\n"
                            f"{row['description'] or 'Product details available on request.'}"
                        )
    ...
    with c1:
        st.markdown("### Need a product enquiry?")
        st.caption(
            "Contact Mahakoshal Refractories for product availability, "
            "technical specifications and business enquiries."
        )
    with c2:
        st.link_button(
...
if not st.session_state.logged_in:
    st.markdown(
        """
        <div class="hero">
            <div style="font-size:.82rem;font-weight:700;letter-spacing:1.5px;opacity:.75;">
                MRPL • MAHAKOSHAL REFRACTORIES
            </div>
            <h1 style="font-size:2.45rem;margin-top:8px;">
                Smart Manufacturing & Quality Management
            </h1>
            <p>
                A professional digital platform for product discovery,
                manufacturing operations, quality management and business workflows.
            </p>
        </div>
        """,
        unsafe_allow_html=True
    ...
    with action1:
        if st.button("🛍️ Explore Our Products", use_container_width=True, type="primary"):
