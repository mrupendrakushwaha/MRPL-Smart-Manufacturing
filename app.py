...
st.markdown(
    ...
    section[data-testid="stSidebar"] {
        background: rgba(3, 18, 34, 0.98) !important;
        border-right: 1px solid rgba(39,168,255,.22);
    }
    section[data-testid="stSidebar"] > div {
        background: linear-gradient(180deg, rgba(5,28,49,.99), rgba(3,17,31,.99)) !important;
    }
    section[data-testid="stSidebar"] .block-container { padding-top: 1.2rem !important; }

    /* Sidebar navigation - compact enterprise menu */
    ...
    section[data-testid="stSidebar"] [role="radiogroup"] > label:hover {
        background: rgba(39,168,255,.09) !important;
        border-color: rgba(39,168,255,.22) !important;
    }
    section[data-testid="stSidebar"] [role="radiogroup"] > label:has(input:checked) {
        background: linear-gradient(90deg, rgba(25,143,225,.25), rgba(25,143,225,.07)) !important;
        border-color: rgba(39,168,255,.38) !important;
        box-shadow: inset 3px 0 0 #27a8ff;
    }
    ...
    /* Sidebar user/role/logout blocks */
    section[data-testid="stSidebar"] .stAlert {
        border-radius: 14px !important;
        border: 1px solid rgba(39,168,255,.20) !important;
        background: rgba(10,43,68,.72) !important;
    }
    section[data-testid="stSidebar"] .stButton > button {
        min-height: 46px !important;
        border-radius: 12px !important;
        background: linear-gradient(135deg,#123f61,#0b2a46) !important;
        border: 1px solid #3b6685 !important;
        color: #fff !important;
    }

    /* Main cards / containers */
    div[data-testid="stVerticalBlockBorderWrapper"] {
        background: rgba(7, 31, 52, 0.93);
        border: 1px solid rgba(52,128,174,.42);
        border-radius: 18px;
        box-shadow: 0 12px 30px rgba(0,0,0,.20);
    }
    div[data-testid="stMetric"] {
        background: linear-gradient(145deg, rgba(13,48,78,.96), rgba(6,29,50,.96));
        border: 1px solid rgba(39,168,255,.26);
        border-radius: 17px;
        padding: 18px 18px;
        box-shadow: 0 8px 24px rgba(0,0,0,.18);
    }
    div[data-testid="stMetricLabel"] { color: #a8bfd1 !important; font-weight: 650 !important; }
    div[data-testid="stMetricValue"] { color: #f5fbff !important; font-weight: 800 !important; }
    ...
    .stButton > button, .stLinkButton > a, .stDownloadButton > button {
        ...
        min-height: 44px;
        font-weight: 750;
        border: 1px solid #2d78a7;
        background: linear-gradient(135deg,#1178bd,#0c4f80);
        color: #fff !important;
    ...
    .home-brand {
        ...
        padding: 18px 20px;
        border:1px solid rgba(110,190,235,.22);
        border-radius:18px;
        background:rgba(5,28,47,.72);
    ...
    .home-logo {
        ...
        font-size:29px;
        background:linear-gradient(135deg,#1889d0,#0b4b78);
        box-shadow:0 8px 20px rgba(0,0,0,.22);
    }
    .home-brand-name {font-size:27px;font-weight:850;color:#fff;line-height:1.05;}
    .home-brand-sub {font-size:12px;color:#a9c0d1;margin-top:5px;letter-spacing:.4px;}
    .home-hero {
        position:relative; overflow:hidden;
        border-radius:24px;
        padding:48px 34px 40px;
        min-height:330px;
        background:linear-gradient(115deg,rgba(3,19,33,.94),rgba(7,46,72,.64)), {MRPL_BG_CSS};
        background-size:cover; background-position:center;
        border:1px solid rgba(103,190,239,.28);
        box-shadow:0 18px 45px rgba(0,0,0,.28);
    }
    .home-hero:after {content:"";position:absolute;width:240px;height:240px;border-radius:50%;right:-80px;top:-90px;background:rgba(39,168,255,.13);}
    .home-kicker {color:#58c2ff;font-size:11px;font-weight:850;letter-spacing:2px;margin-bottom:13px;}
    .home-title {color:#fff;font-size:42px;font-weight:900;line-height:1.08;max-width:720px;margin-bottom:12px;}
    .home-copy {color:#c5d8e6;font-size:15px;line-height:1.65;max-width:650px;}
    .home-pills {display:flex;flex-wrap:wrap;gap:9px;margin-top:22px;}
    .home-pill {padding:8px 12px;border-radius:999px;background:rgba(255,255,255,.08);border:1px solid rgba(160,210,235,.20);color:#eaf5fb;font-size:12px;font-weight:650;}
    .home-section-title {text-align:center;color:#fff;font-size:21px;font-weight:800;margin:28px 0 12px;}
    .home-footer {text-align:center;color:#9db5c8;font-size:12px;margin-top:20px;letter-spacing:.3px;}

    /* Inputs */
    div[data-baseweb="input"], div[data-baseweb="select"], textarea, input {
        background: #0b223b !important;
        color: #eef6ff !important;
        border-color: #1d4264 !important;
    }
    ...
    hr {{ border-color: #1b3d5d !important; }}
...
if not st.session_state.logged_in:
    st.markdown(
        ...
        <div class="home-shell">
            <div class="home-brand">
                ...
                <div>
                    <div class="home-brand-name">MRPL</div>
                    <div class="home-brand-sub">Mahakoshal Refractories Pvt. Ltd.</div>
                </div>
            </div>

            <div class="home-hero">
                <div class="home-kicker">SMART MANUFACTURING • QUALITY • OPERATIONS</div>
                <div class="home-title">MRPL Smart Manufacturing System</div>
                <div class="home-copy">
                    A centralized platform for manufacturing operations, quality control,
                    inventory, orders, dispatch and workforce management.
                </div>
                <div class="home-pills">
                    <span class="home-pill">⚙️ Smart Operations</span>
                    <span class="home-pill">📊 Better Productivity</span>
                    <span class="home-pill">🌱 Sustainable Growth</span>
                </div>
            </div>

            <div class="home-section-title">Access MRPL Services</div>
        </div>
        """,
        unsafe_allow_html=True
    )

    # Keep Streamlit buttons for the existing actions; CSS gives them the new design.
    login_btn = st.button("🔐  Staff Login  →", use_container_width=True, key="home_login")
    products_btn = st.button("🛍️  Explore Our Products  →", use_container_width=True, key="home_products")
    ...
    if login_btn or st.session_state.get("show_login", False):
        ...
        st.markdown("<div class='home-section-title'>Secure Staff Login</div>", unsafe_allow_html=True)

        with st.form("login_form"):
            username = st.text_input("Username", placeholder="Enter username")
            password = st.text_input("Password", type="password", placeholder="Enter password")
            login_submit = st.form_submit_button("🔑  Login to MRPL", use_container_width=True)
    ...
    st.markdown(
        "<div class='home-footer'>Secure • Efficient • Reliable • Sustainable</div>",
        unsafe_allow_html=True
    )
    st.stop()
...
st.sidebar.markdown(
    ...
    <div style="padding:4px 0 16px;">
      <div style="display:flex;align-items:center;gap:11px;">
        <div style="width:48px;height:48px;border-radius:14px;background:linear-gradient(135deg,#1689d4,#0a4b78);display:flex;align-items:center;justify-content:center;font-size:24px;">🏭</div>
        <div>
          <div style="font-size:22px;font-weight:850;color:#fff;line-height:1;">MRPL</div>
          <div style="font-size:10px;color:#91abc0;letter-spacing:1px;margin-top:5px;">SMART MANUFACTURING</div>
        </div>
      </div>
    </div>
    <div style="height:1px;background:rgba(115,178,215,.16);margin:0 0 18px;"></div>
    <div style="padding:14px;border:1px solid rgba(39,168,255,.25);border-radius:16px;background:linear-gradient(135deg,rgba(12,62,91,.72),rgba(6,34,55,.75));">
      <div style="display:flex;align-items:center;gap:12px;">
        <div style="width:48px;height:48px;border-radius:50%;background:linear-gradient(135deg,#20a7f0,#10639b);display:flex;align-items:center;justify-content:center;font-size:23px;">👤</div>
        <div style="flex:1;">
          <div style="font-size:16px;font-weight:800;color:#fff;">{username}</div>
          <div style="font-size:12px;color:#a9bfd0;margin-top:4px;">{user_role}</div>
        </div>
        <div style="font-size:11px;color:#5bea88;font-weight:750;">● Online</div>
      </div>
    </div>
    """,
    unsafe_allow_html=True
