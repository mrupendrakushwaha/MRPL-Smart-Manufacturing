...
def _secret_value(key, default=""):
    try:
        ...
        if hasattr(admin_section, "get"):
            value = admin_section.get(key, default)
...
def ensure_schema():
    """Create/repair the small core schema used by the application."""
    ...
    statements = [
        ...
        """CREATE TABLE IF NOT EXISTS employees (
            id INTEGER PRIMARY KEY AUTOINCREMENT, employee_code TEXT UNIQUE NOT NULL,
            name TEXT NOT NULL, department_id INTEGER, designation TEXT, phone TEXT,
            email TEXT, joining_date TEXT, status TEXT NOT NULL DEFAULT 'Active'
        )""",
        """CREATE TABLE IF NOT EXISTS attendance (
            id INTEGER PRIMARY KEY AUTOINCREMENT, employee_id INTEGER NOT NULL,
            employee_name TEXT, att_date TEXT NOT NULL, status TEXT NOT NULL,
            check_in TEXT, check_out TEXT, UNIQUE(employee_id, att_date)
        )""",
        """CREATE TABLE IF NOT EXISTS leave_requests (
            id INTEGER PRIMARY KEY AUTOINCREMENT, employee_id INTEGER, employee_name TEXT,
            leave_type TEXT, start_date TEXT, end_date TEXT, reason TEXT, status TEXT DEFAULT 'Pending'
        )""",
        """CREATE TABLE IF NOT EXISTS tasks (
            id INTEGER PRIMARY KEY AUTOINCREMENT, employee_id INTEGER, employee_name TEXT,
            title TEXT, description TEXT, due_date TEXT, status TEXT DEFAULT 'Pending'
        )""",
        ...
        """CREATE TABLE IF NOT EXISTS orders (
            id INTEGER PRIMARY KEY AUTOINCREMENT, customer_name TEXT, product_id INTEGER,
            quantity REAL, order_date TEXT, delivery_date TEXT, status TEXT DEFAULT 'Pending'
        )""",
        """CREATE TABLE IF NOT EXISTS dispatch (
            id INTEGER PRIMARY KEY AUTOINCREMENT, order_id INTEGER, quantity REAL,
            dispatch_date TEXT, vehicle_number TEXT, transporter TEXT, status TEXT DEFAULT 'Pending'
        )""",
        """CREATE TABLE IF NOT EXISTS raw_materials (
            id INTEGER PRIMARY KEY AUTOINCREMENT, material_name TEXT, category TEXT,
            current_stock REAL DEFAULT 0, minimum_stock REAL DEFAULT 0, unit TEXT,
            supplier TEXT, unit_price REAL DEFAULT 0, last_updated TEXT
        )""",
        """CREATE TABLE IF NOT EXISTS quality_checks (
            id INTEGER PRIMARY KEY AUTOINCREMENT, batch_no TEXT, product_id INTEGER,
            product_name TEXT, test_date TEXT, parameter TEXT, result TEXT, status TEXT,
            inspector TEXT, remarks TEXT
        )""",
        """CREATE TABLE IF NOT EXISTS production (
            id INTEGER PRIMARY KEY AUTOINCREMENT, raw_material_id INTEGER,
            required_quantity REAL DEFAULT 0, product_id INTEGER, produced_quantity REAL DEFAULT 0,
            date TEXT, quantity REAL, batch_no TEXT, product_name TEXT, status TEXT
        )""",
        """CREATE TABLE IF NOT EXISTS products (
            id INTEGER PRIMARY KEY AUTOINCREMENT, category TEXT, product_name TEXT,
            description TEXT, image_url TEXT, is_active INTEGER DEFAULT 1
        )"""
    ]
    for stmt in statements:
        cur.execute(stmt)
    # Repair common old-schema differences without destroying data.
    migrations = {
        "users": [("is_active", "INTEGER NOT NULL DEFAULT 1"), ("status", "TEXT NOT NULL DEFAULT 'Active'"), ("employee_id", "INTEGER")],
        "employees": [("employee_code", "TEXT"), ("name", "TEXT"), ("department_id", "INTEGER"), ("designation", "TEXT"), ("phone", "TEXT"), ("email", "TEXT"), ("joining_date", "TEXT"), ("status", "TEXT DEFAULT 'Active'")],
        "attendance": [("employee_name", "TEXT"), ("att_date", "TEXT"), ("status", "TEXT"), ("check_in", "TEXT"), ("check_out", "TEXT")],
    }
        ...
        for col, definition in cols:
            if col not in existing:
                try:
                    ...
                    pass
    # Keep legacy users usable when only status exists.
    cur.execute("UPDATE users SET is_active = CASE WHEN status = 'Inactive' THEN 0 ELSE 1 END WHERE is_active IS NULL")
    cur.execute("UPDATE users SET status = CASE WHEN is_active = 0 THEN 'Inactive' ELSE 'Active' END WHERE status IS NULL OR status = ''")
    cur.execute("INSERT OR IGNORE INTO departments(name) VALUES ('Administration')")
    cur.execute("INSERT OR IGNORE INTO departments(name) VALUES ('Production')")
    cur.execute("INSERT OR IGNORE INTO departments(name) VALUES ('Quality')")
    cur.execute("INSERT OR IGNORE INTO departments(name) VALUES ('HR')")
    conn.commit()
    conn.close()


def ensure_bootstrap_admin():
    """Create the first administrator from Streamlit private Secrets only."""
    username = _secret_value("ADMIN_USERNAME", "") .strip()
    password = _secret_value("ADMIN_PASSWORD", "")
    ...
    if existing is None:
        cur.execute("INSERT INTO users(username,password_hash,role,employee_id,is_active,status) VALUES (?,?,?,?,?,?)",
                    (username, hash_password(password), "Admin", None, 1, "Active"))
    else:
        cur.execute("UPDATE users SET role='Admin', is_active=1, status='Active' WHERE id=?", (existing[0],))
    conn.commit()
    conn.close()
...
st.sidebar.markdown("## 🏭 MRPL Smart")
st.sidebar.caption("Manufacturing & Quality Management")
st.sidebar.success(f"👤 {current_user[1]}")
st.sidebar.info(f"🛡️ Role: {current_user[3]}")


st.markdown("""
<div style="padding:22px 24px;border-radius:18px;background:linear-gradient(135deg,#0f2747,#123f68);color:white;margin-bottom:18px;box-shadow:0 8px 24px rgba(15,39,71,.12)">
<div style="font-size:30px;font-weight:800">🏭 MRPL Smart Manufacturing</div>
<div style="opacity:.85;margin-top:4px">Manufacturing • Quality • Inventory • Workforce • Analytics</div>
</div>
...
def get_employees():
    if not table_exists("employees"):
        return pd.DataFrame()
    return fetch_dataframe(
        """SELECT id, employee_code, name, department_id, designation, phone, email, joining_date, status
...
def get_attendance(employee_id=None):
    ...
    if employee_id is None:
        return fetch_dataframe(
            """SELECT a.id, a.employee_id, COALESCE(a.employee_name, e.name) AS employee_name,
                      a.att_date, a.status, a.check_in, a.check_out
    ...
    return fetch_dataframe(
        """SELECT a.id, a.employee_id, COALESCE(a.employee_name, e.name) AS employee_name,
                  a.att_date, a.status, a.check_in, a.check_out
           FROM attendance a LEFT JOIN employees e ON e.id=a.employee_id
           WHERE a.employee_id=? ORDER BY a.att_date DESC, a.id DESC""",
        (employee_id,)
    )
...
def mark_attendance(employee_id, attendance_date, status, check_in=None, check_out=None):
    ...
    employee = fetch_one("SELECT name FROM employees WHERE id=?", (employee_id,))
    if not employee:
        return False, "Employee profile not found. Link the user to an employee first."
    try:
        execute_query(
            """INSERT INTO attendance(employee_id, employee_name, att_date, status, check_in, check_out)
               VALUES (?,?,?,?,?,?)""",
            (employee_id, employee[0], attendance_date, status, check_in, check_out)
        )
        return True, "Attendance marked successfully."
    except sqlite3.IntegrityError:
        return False, "Attendance already exists for this employee on this date."
    except sqlite3.Error as e:
        return False, str(e)
...
if selected_module == "Attendance":
    st.header("🕐 Attendance Management")
    st.caption("Track daily attendance, check-in/out and attendance history.")

    is_self_view = user_role not in ["Admin", "Manager", "HR"]
    linked_employee_id = current_user[4]

    if is_self_view and not linked_employee_id:
        st.warning("👤 No employee profile is linked to this account.")
        st.info("Ask an Admin/HR user to link this account to your employee profile from User Management.")
    else:
        if is_self_view:
            employees = get_employees()
            my_employee = employees[employees["id"] == linked_employee_id]
            if my_employee.empty:
                st.error("Linked employee profile was not found. Please contact Admin/HR.")
            else:
                emp_name = my_employee.iloc[0]["name"]
                st.markdown(f"### 👋 {emp_name}")
                today = date.today().isoformat()
                existing = fetch_one("SELECT id,status,check_in,check_out FROM attendance WHERE employee_id=? AND att_date=?", (linked_employee_id,today))
                if existing:
                    st.success(f"Today: {existing[1]} • Check-in: {existing[2] or '—'} • Check-out: {existing[3] or '—'}")
                else:
                    if st.button("✅ Check-in Today", type="primary", use_container_width=True):
                        now = datetime.now().strftime("%H:%M")
                        ok,msg = mark_attendance(linked_employee_id,today,"Present",now,None)
                        (st.success if ok else st.error)(msg)
                        if ok: st.rerun()
                mine = get_attendance(linked_employee_id)
                if mine.empty: st.info("No attendance records yet.")
            ...
            if employees.empty:
                ...
                employee_options = {f"{r['employee_code']} — {r['name']}": int(r['id']) for _,r in employees.iterrows()}
                with st.container(border=True):
                    st.subheader("➕ Mark Attendance")
                    with st.form("attendance_form"):
                        employee_name = st.selectbox("Employee", list(employee_options.keys()))
                        col1,col2 = st.columns(2)
                        with col1: attendance_date = st.date_input("Attendance Date", value=date.today())
                        with col2: status = st.selectbox("Status", ["Present","Absent","Leave"])
                        c1,c2 = st.columns(2)
                        with c1: check_in = st.text_input("Check In", placeholder="09:00")
                        with c2: check_out = st.text_input("Check Out", placeholder="18:00")
                        submitted = st.form_submit_button("✅ Save Attendance", type="primary", use_container_width=True)
                        if submitted:
                            ok,msg = mark_attendance(employee_options[employee_name], attendance_date.isoformat(), status, check_in.strip() or None, check_out.strip() or None)
                            (st.success if ok else st.error)(msg)
                            if ok: st.rerun()
                st.divider()
                st.subheader("📋 Attendance Records")
                attendance = get_attendance()
                if attendance.empty: st.info("No attendance records available.")
                else: st.dataframe(attendance, use_container_width=True, hide_index=True)
...
def get_users():
    if not table_exists("users"):
        return pd.DataFrame()
    return fetch_dataframe("SELECT id, username, role, employee_id, status FROM users ORDER BY id DESC")


def count_admins():
    row = fetch_one("SELECT COUNT(*) FROM users WHERE role='Admin' AND is_active=1")
...
def create_user(username, password, role, employee_id=None):
    ...
    if role not in ["Admin","Manager","Production","Quality","Warehouse","Sales","Dispatch","HR","Employee"]:
        return False, "Invalid role."
    try:
        execute_query("INSERT INTO users(username,password_hash,role,employee_id,is_active,status) VALUES (?,?,?,?,?,?)",
                      (username,hash_password(password),role,employee_id,1,"Active"))
...
def update_user_role(user_id, role):
    valid_roles=["Admin","Manager","Production","Quality","Warehouse","Sales","Dispatch","HR","Employee"]
    if role not in valid_roles: return False,"Invalid role."
    try:
        execute_query("UPDATE users SET role=? WHERE id=?",(role,user_id)); return True,"User role updated successfully."
    except sqlite3.Error as e: return False,str(e)


def update_user_status(user_id, status):
    if status not in ["Active","Inactive"]: return False,"Invalid user status."
    try:
        execute_query("UPDATE users SET status=?, is_active=? WHERE id=?",(status,1 if status=="Active" else 0,user_id)); return True,"User status updated successfully."
    except sqlite3.Error as e: return False,str(e)


def link_user_employee(user_id, employee_id):
    try:
        execute_query("UPDATE users SET employee_id=? WHERE id=?",(employee_id,user_id)); return True,"Employee profile linked successfully."
    except sqlite3.Error as e: return False,str(e)
...
if selected_module == "Users":
    ...
    st.caption("Manage administrators, user roles, account status and employee links.")
    ...
    m2.metric("Active Users", int((users["status"]=="Active").sum()) if not users.empty else 0)
    m3.metric("Active Admins", admin_count)

    st.divider()
    with st.container(border=True):
        st.subheader("➕ Add Another Admin")
        st.caption("The first Admin is created from Manage App → Secrets. Additional Admins are created here.")
        with st.form("add_admin_form"):
            c1,c2 = st.columns(2)
            with c1: new_admin_username=st.text_input("Admin Username")
            with c2: new_admin_password=st.text_input("Admin Password",type="password")
            if st.form_submit_button("🛡️ Create Admin",type="primary",use_container_width=True):
                ok,msg=create_user(new_admin_username,new_admin_password,"Admin",None)
                (st.success if ok else st.error)(msg)
                if ok: st.rerun()
    ...
    if not users.empty:
        ...
        c1,c2 = st.columns(2)
        with c1:
            selected_user=st.selectbox("Select User",users["id"].tolist(),format_func=lambda x: users.loc[users.id==x,"username"].iloc[0])
            new_role=st.selectbox("Role",["Admin","Manager","Production","Quality","Warehouse","Sales","Dispatch","HR","Employee"])
            new_status=st.selectbox("Status",["Active","Inactive"])
            if st.button("💾 Update User",type="primary",use_container_width=True):
                if selected_user==current_user[0] and new_status=="Inactive":
                    st.error("You cannot deactivate your own account.")
                elif selected_user==current_user[0] and new_role!="Admin":
                    st.error("You cannot remove your own Admin role while logged in.")
                else:
                    ok1,msg1=update_user_role(selected_user,new_role); ok2,msg2=update_user_status(selected_user,new_status)
                    if ok1 and ok2: st.success("User updated successfully."); st.rerun()
                    else: st.error(msg1 if not ok1 else msg2)
        with c2:
            st.subheader("🔗 Link Employee Profile")
            if employees.empty:
                st.info("Add an employee first.")
            else:
                emp_id=st.selectbox("Employee",employees["id"].tolist(),format_func=lambda x: f"{employees.loc[employees.id==x,'employee_code'].iloc[0]} — {employees.loc[employees.id==x,'name'].iloc[0]}")
                if st.button("🔗 Link Selected User",use_container_width=True):
                    ok,msg=link_user_employee(selected_user,emp_id); (st.success if ok else st.error)(msg)
                    if ok: st.rerun()
...
st.markdown(
    ...
    [data-testid="stSidebar"] { background: #0b2341; }
    [data-testid="stSidebar"] * { color: #f8fafc !important; }
    [data-testid="stSidebar"] .stRadio label { padding: 7px 8px; border-radius: 8px; }
    div[data-testid="stVerticalBlockBorderWrapper"] { border-radius: 16px; border: 1px solid #e7ebf2; box-shadow: 0 4px 18px rgba(15,39,71,.05); }
