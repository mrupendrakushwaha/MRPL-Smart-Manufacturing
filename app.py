...
st.markdown(
    ...
    .stApp {
        background: linear-gradient(180deg, #061525 0%, #071a2d 100%);
        color: var(--mrpl-text);
    }
    ...
    /* Sidebar - industrial control-panel look */
    section[data-testid="stSidebar"] {
        background: #071b30;
        border-right: 1px solid var(--mrpl-border);
    }
    section[data-testid="stSidebar"] > div {
        background: linear-gradient(180deg, #081d34 0%, #061525 100%);
    }
    ...
    div[data-testid="stMetric"] {
        background: linear-gradient(145deg, #0d2a47, #0a2037);
        border: 1px solid #1b4468;
        border-radius: 12px;
        padding: 14px 16px;
        box-shadow: 0 6px 18px rgba(0,0,0,.18);
    }
    div[data-testid="stMetricLabel"] { color: #9eb4ca !important; }
    div[data-testid="stMetricValue"] { color: #f2f8ff !important; }
    ...
    .stDownloadButton > button {
        ...
        background: linear-gradient(135deg, #0d5d95, #0b416d);
        color: #ffffff !important;
        box-shadow: 0 5px 14px rgba(0,0,0,.18);
    }
    .stButton > button:hover, .stLinkButton > a:hover {
        border-color: #39b6ff;
        background: linear-gradient(135deg, #1175b8, #0d4f80);
    }
    ...
    @media (max-width: 768px) {
        .block-container { padding: 1.1rem .8rem 1.5rem .8rem; }
...
def init_db():
    ...
    # Product photos are stored in this public GitHub repository.
    # Store GitHub RAW URLs in the database so Streamlit Cloud can
    # display the images even when the image folders are not mounted
    # next to app.py in the deployed runtime.
    # --------------------------------------------------------

    GH_RAW_BASE = (
        "https://raw.githubusercontent.com/"
        "mrupendrakushwaha/MRPL-Smart-Manufacturing/main"
    )

    def _gh_raw_url(*parts):
        """Build a GitHub RAW URL while safely encoding each path part."""
        from urllib.parse import quote
        return GH_RAW_BASE + "/" + "/".join(quote(str(p), safe="") for p in parts)

    def product_image(*parts):
        """Return the public GitHub RAW URL for one product image."""
        return _gh_raw_url(*parts)

    def product_images(folder, filenames):
        """Return GitHub RAW URLs for product images as CSV."""
        return ",".join(
            _gh_raw_url(folder, filename)
            for filename in filenames
        )

    # Bump this whenever the product catalog/image mapping changes.
    product_refresh_key = "mrpl_official_products_v7"
        ...
        for i, (_, row) in enumerate(cat_products.iterrows()):
            with cols[i % 3]:
                with st.container(border=True):
                    ...
                    if image_urls:
                        # image_url values are public GitHub RAW URLs.
                        # Do NOT use Path(...).is_file() here: a URL is not a
                        # local filesystem path, so Path(url).is_file() is
                        # always False on Streamlit Cloud.
                        try:
                            st.image(image_urls[0], width="stretch")
                            if len(image_urls) > 1:
                                thumb_cols = st.columns(len(image_urls) - 1)
                                for tc, extra_url in zip(thumb_cols, image_urls[1:]):
                                    with tc:
                                        st.image(extra_url, width="stretch")
                        except Exception:
                            st.info("Product image could not be loaded from GitHub.")
