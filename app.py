...
def init_db():
    ...
    # Users are NOT seeded with public/demo passwords.
    # The first Admin is created securely from Streamlit Secrets below.
...
def ensure_bootstrap_admin():
    """
    Create the first Admin from Streamlit Secrets.
    ...
    On first setup, old demo login accounts (admin/manager/employee)
    are removed so they cannot remain as hidden default logins.
    """
    try:
        cfg = st.secrets.get("bootstrap_admin", {})
        bootstrap_username = str(cfg.get("username", "")).strip()
        bootstrap_password = str(cfg.get("password", ""))
    except Exception:
        bootstrap_username = ""
        bootstrap_password = ""
    ...
    # Remove only the old hard-coded demo accounts.
    # Do not touch employee records or any other real user accounts.
    legacy_names = ("admin", "manager", "employee")
    placeholders = ",".join(["?"] * len(legacy_names))
    delete_params = tuple(name for name in legacy_names if name != bootstrap_username)

    if delete_params:
        placeholders = ",".join(["?"] * len(delete_params))
        conn.execute(
            f"DELETE FROM users WHERE username IN ({placeholders})",
            delete_params
        )

    # If the configured bootstrap username already exists, make that
    # account the Admin using the Secret password. Otherwise create it.
    ...
    if existing:
        conn.execute(
            ...
            WHERE id = ?
            """,
            (hash_password(bootstrap_password), existing[0])
        )
    else:
        conn.execute(
            """
...
# ------------------------------------------------------------
    ...
    st.dataframe(my_att, width="stretch", hide_index=True)
