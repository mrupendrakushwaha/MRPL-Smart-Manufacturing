...
# ============================================================
# PROFESSIONAL MRPL UI - visual layer only
# ============================================================
st.markdown(
    """
    <style>
    :root {
        ...
        --mrpl-blue-2: #0878c9;
        --mrpl-cyan: #22b8e6;
        --mrpl-border: rgba(83, 178, 235, .28);
        --mrpl-text: #f4f8fc;
        --mrpl-muted: #9fb4c7;
    }

    .stApp {
        background: radial-gradient(circle at 80% 10%, rgba(21,151,229,.13), transparent 30%),
                    linear-gradient(180deg, #061625 0%, #0a1e31 100%);
    }
    ...
    .mrpl-header {
        ...
        border-radius: 18px;
        background: linear-gradient(135deg, rgba(10,42,70,.96), rgba(7,26,45,.96));
    ...
    .mrpl-logo {
        ...
        box-shadow: 0 8px 18px rgba(239,83,80,.22);
    }
    .mrpl-brand { font-size: 25px; font-weight: 850; line-height: 1.05; }
    .mrpl-sub { color: var(--mrpl-muted); font-size: 13px; margin-top: 4px; }

    /* ---------- Sidebar ---------- */
    section[data-testid="stSidebar"] {
        background: #06192b;
        border-right: 1px solid var(--mrpl-border);
    }
    section[data-testid="stSidebar"] > div {
        background: linear-gradient(180deg, #071d32 0%, #041322 100%);
    }
    section[data-testid="stSidebar"] .block-container { padding-top: 1rem; }

    .sidebar-brand {
        padding: 6px 2px 18px;
        border-bottom: 1px solid rgba(120,190,230,.18);
        margin-bottom: 18px;
    }
    .sidebar-brand .name { font-size: 24px; font-weight: 850; }
    .sidebar-brand .sub { color: #91a9bc; font-size: 11px; letter-spacing: .7px; margin-top: 3px; }

    .profile-card {
        border: 1px solid rgba(34,184,230,.30);
        border-radius: 17px;
        padding: 14px;
        background: linear-gradient(135deg, #0b304b, #08223a);
        margin-bottom: 12px;
    }
    ...
    .avatar {
        ...
        background: linear-gradient(135deg,#1597e5,#0878c9);
        font-size: 24px;
    }
    .profile-name { font-weight:800; font-size:16px; }
    .profile-role { color:#9fb4c7; font-size:13px; margin-top:3px; }
    .online { margin-left:auto; color:#64e58b; font-size:12px; font-weight:700; }
    ...
    section[data-testid="stSidebar"] [data-testid="stRadio"] div[role="radiogroup"] > label:hover {
        ...
        border-color: rgba(21,151,229,.20);
    }
    section[data-testid="stSidebar"] [data-testid="stRadio"] div[role="radiogroup"] > label:has(input:checked) {
        background: linear-gradient(90deg, rgba(21,151,229,.23), rgba(21,151,229,.07));
        border-color: rgba(34,184,230,.42);
        box-shadow: inset 3px 0 0 #22b8e6;
    }
    ...
    .stButton > button, .stLinkButton > a, .stDownloadButton > button {
        min-height: 44px;
        border-radius: 12px;
        border: 1px solid rgba(76,181,237,.38);
        background: linear-gradient(135deg, #1597e5, #0878c9);
        color: #fff !important;
        font-weight: 750;
    ...
    div[data-testid="stMetric"] {
        background: linear-gradient(145deg, #0c2d4b, #081f36);
        border: 1px solid rgba(73,171,228,.28);
        border-radius: 16px;
        padding: 17px 18px;
        min-height: 108px;
        box-shadow: 0 10px 24px rgba(0,0,0,.16);
    }
    div[data-testid="stMetricLabel"] { color:#9fb4c7 !important; }
    ...
    div[data-baseweb="input"], div[data-baseweb="select"], textarea, input {
        ...
        border-color: #1e4b6c !important;
    }
    div[data-baseweb="select"] * { color:#f4f8fc !important; }
    ...
    div[data-testid="stVerticalBlockBorderWrapper"] {
        background: rgba(9,35,58,.78);
        border:1px solid rgba(73,171,228,.24);
        border-radius:15px;
    }
    hr { border-color: rgba(100,180,225,.18) !important; }

    /* ---------- Landing page ---------- */
    .landing {
        border:1px solid rgba(73,171,228,.25);
        border-radius:24px;
        padding: 30px 24px 22px;
        background: linear-gradient(145deg, rgba(10,39,64,.94), rgba(5,22,38,.96));
        box-shadow: 0 18px 40px rgba(0,0,0,.22);
        margin-bottom: 16px;
    }
    .landing-kicker { color:#55c8ff; font-size:12px; font-weight:800; letter-spacing:2px; text-transform:uppercase; }
    .landing-title { font-size:32px; font-weight:850; line-height:1.08; margin:8px 0; }
    .landing-text { color:#a9bdcf; font-size:14px; line-height:1.55; max-width:700px; }
    .feature-row { display:flex; flex-wrap:wrap; gap:8px; margin-top:20px; }
    .feature { padding:8px 11px; border:1px solid rgba(73,171,228,.20); border-radius:999px; color:#c9d9e6; background:rgba(21,151,229,.07); font-size:12px; }

    @media (max-width: 768px) {
        .block-container { padding: .7rem .65rem 1.4rem; }
        .mrpl-header { padding:12px 13px; border-radius:15px; }
        .mrpl-logo { width:46px; height:46px; font-size:23px; }
        .mrpl-brand { font-size:21px; }
        .landing { padding:23px 17px 18px; border-radius:19px; }
        ...
        for i, (_, row) in enumerate(cat_products.iterrows()):
            with cols[i % 3]:
                with st.container(border=True):
                    image_urls = [u.strip() for u in str(row["image_url"] or "").split(",") if u.strip()]
                    if image_urls:
                        # Only render an image when the local file actually exists.
                        valid_images = [u for u in image_urls if Path(u).is_file()]
                        if valid_images:
                            st.image(valid_images[0], width="stretch")
                            if len(valid_images) > 1:
                                thumb_cols = st.columns(len(valid_images) - 1)
                                for tc, extra_url in zip(thumb_cols, valid_images[1:]):
                                    with tc:
                                        st.image(extra_url, width="stretch")
                        else:
                            st.info("Product image is not available in the deployed repository.")
...
if not st.session_state.logged_in:
    st.markdown(
        ...
        <div class="mrpl-header">
            <div class="mrpl-logo">🏭</div>
            <div>
                <div class="mrpl-brand">MRPL</div>
                <div class="mrpl-sub">Mahakoshal Refractories Pvt. Ltd.</div>
            </div>
        </div>
        """,
        unsafe_allow_html=True,
    ...
    st.markdown(
        """
        <div class="landing">
            <div class="landing-kicker">SMART MANUFACTURING PLATFORM</div>
            <div class="landing-title">MRPL Smart Manufacturing System</div>
            <div class="landing-text">
                A centralized workspace for manufacturing operations, quality,
                inventory, orders, dispatch and employee management.
            </div>
            <div class="feature-row">
                <span class="feature">⚙️ Smart Operations</span>
                <span class="feature">▣ Better Productivity</span>
                <span class="feature">🌿 Sustainable Growth</span>
            </div>
        </div>
        """,
        unsafe_allow_html=True,
    ...
    with col2:
        products_btn = st.button("🛍️  Our Products  →", use_container_width=True)
    ...
    st.markdown(
        '<div style="text-align:center;color:#8fa8bb;font-size:12px;margin-top:22px;">Secure • Efficient • Reliable • Sustainable</div>',
        unsafe_allow_html=True,
    )
    st.stop()
...
st.sidebar.markdown(
    ...
    <div class="sidebar-brand">
        <div class="name">🏭 MRPL</div>
        <div class="sub">SMART MANUFACTURING SYSTEM</div>
    </div>
    """,
    unsafe_allow_html=True,
)
...
st.markdown(
    ...
    <div class="mrpl-header">
        <div class="mrpl-logo">🏭</div>
        <div>
            <div class="mrpl-brand">MRPL Smart Manufacturing System</div>
            <div class="mrpl-sub">Manufacturing • Quality • Inventory • Dispatch</div>
        </div>
    </div>
    """,
    unsafe_allow_html=True,
